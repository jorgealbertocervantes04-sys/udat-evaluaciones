...
export default function GestorEvaluaciones({ evaluadorEmail }) {
  ...
  useEffect(() => { setRespuestasChecklist({}); }, [tipoEvaluacion]);
  ...
  const limpiarFormulario = () => {
    ...
    setCompromisoMejora('');
    firmaMentorRef.current.clear();
    firmaEvaluadorRef.current.clear();
  };

  // TRANSMISIÓN (CONECTADA A TU SUPABASE ORIGINAL)
  const procesarEvaluacion = async (datosFormulario) => {
    if (isOnline) {
      // AQUÍ ESTÁ EL AJUSTE FINAL: Mandamos directo a tu tabla 'evaluaciones'
      const { error } = await supabase.from('evaluaciones').insert([datosFormulario]);
      if (error) {
        alert("Falla de red, guardando en la bitácora local por seguridad.");
        guardarLocal(datosFormulario);
      } else {
        alert("Evaluación registrada exitosamente en la base central.");
        limpiarFormulario();
      }
    } else {
      guardarLocal(datosFormulario);
  ...
  const guardarLocal = (datosFormulario) => {
    ...
    alert("Modo offline: Evaluación asegurada en la tablet.");
    limpiarFormulario();
  };
  ...
  async function sincronizarDatosPendientes() {
        ...
        // RESTAURADO TAMBIÉN AQUÍ A 'evaluaciones'
...
const limpiarFirmaEvaluador = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (contestadas < total) {
      alert(`Faltan evaluar ${total - contestadas} rubros del checklist técnico.`);
      return;
    }
    if (firmaMentorRef.current.isEmpty() || firmaEvaluadorRef.current.isEmpty()) {
      alert("Faltan las firmas de validación.");
      return;
    }
    if (!compromisoMejora) {
      alert("El plan de acción y mejora continua es obligatorio para el cierre.");
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div>
                <label className="block font-bold text-slate-500 text-xs uppercase tracking-wider mb-2">Módulo Operativo:</label>
        ...
        <button type="submit" disabled={contestadas < total} className="w-full bg-slate-900 text-white p-6 rounded-2xl text-xl font-black hover:bg-blue-700 transition-all shadow-xl uppercase tracking-widest disabled:opacity-50 disabled:cursor-not-allowed">
          {contestadas < total ? `Faltan ${total - contestadas} rubros por evaluar` : `Sellar Evaluación de ${tipoEvaluacion}`}
        </button>
      </form>
    </div>
  );
